...
with open('dpNewRecords.txt') as dp_records:
	...
	for line in file:
		if "~~~" not in line:
			...
			current_string = current_string.strip().replace('\n', ' ').replace('\r',' ').replace('\t', ' ').lower()
		#Removes all symbols
		for char in symbols:
			current_string = stripped_description.replace(char, ' ')
		current_string = stripped_description.replace('      ', ' ').replace('    ', ' ').replace('   ', ' ').replace('  ', ' ').strip()
